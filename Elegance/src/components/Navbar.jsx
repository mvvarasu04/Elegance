    ...
    <nav className="bg-blue-200 shadow-md px-4 py-2 flex justify-between items-center relative">
      ...
      <div className="hidden md:flex gap-6 text-lg font-medium items-center relative">
        ...
        >
          ...
          {showServices && (
            <div className="absolute top-full left-0 mt-2 bg-white shadow-lg rounded-md z-20 w-64">
              {servicesList.map((service) => (
                <NavLink
                  key={service.path}
                  to={service.path}
                  className="block px-4 py-2 text-violet-800 hover:bg-blue-100 hover:text-green-600"
                >
                  {service.name}
                </NavLink>
              ))}
